...
def affine_forward(x, w, b):
  ...
  #############################################################################
  x_stretched = x.reshape(x.shape[0], -1)
...
def affine_backward(dout, cache):
  ...
  #############################################################################
  db = np.sum(dout,0)
  dw = x_stretched.T.dot(dout)
  dx_stretched = dout.dot(w.T)
  dx = dx_stretched.reshape(x.shape)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx, dw, db
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    ###########################################################################
    p_pass = 1-p
    mask=np.random.random_sample(x.shape)<p_pass
    out=x*mask/p_pass
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    for f in range(0,F):
      for hp in range(0,Hprime):
        for wp in range(0,Wprime):
          ...
          wps=wp*stride
          wps1=(wp+1)*stride
          out[n,f,hp,wp]=np.sum(w[f,:,:,:]*padded_x[n,:,hp*stride:(hp*stride+HH),wp*stride:(wp*stride+WW)])+b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  N,C,H,W=x.shape
  x_n = np.reshape(np.swapaxes(x,1,3),[N*H*W,C])
  x_n_normed,cache = batchnorm_forward(x_n,gamma,beta,bn_param)
  out = np.swapaxes(x_n_normed.reshape([N,W,H,C]),1,3)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def spatial_batchnorm_backward(dout, cache):
  ...
  #############################################################################
  N,C,H,W=dout.shape
  dout_n = np.reshape(np.swapaxes(dout,1,3),[N*H*W,C])
  dx_n,dgamma,dbeta=batchnorm_backward(dout_n,cache)
  dx = np.swapaxes(dx_n.reshape([N,W,H,C]),1,3)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
